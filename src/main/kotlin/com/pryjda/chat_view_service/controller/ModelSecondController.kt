...
class ModelSecondController {
    ...
    @GetMapping("/t")
    fun increase2(@RequestParam(value = "type") type: String, @RequestParam(value = "priority") priority: String): Statistics {
        when (type) {
            "inform" -> {
                when (priority) {
                    "high" -> {
                        list[0].statistics.inform.high++
                    }
                    "normal" -> {
                        list[0].statistics.inform.normal++
                    }
                }
            }
            "code" -> {
                when (priority) {
                    "high" -> {
                        list[0].statistics.code.high++
                    }
                    "normal" -> {
                        list[0].statistics.code.normal++
                    }
                }
            }
        }
